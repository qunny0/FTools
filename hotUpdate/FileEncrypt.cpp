...
FileEncrypt::FileEncrypt(const char* outputpath) : _outputPath(outputpath)
{
...
//    }
    ...
    {
		// std::cout << "[33]" << folderPath.c_str() << std::endl;
        encryptFileByName(folderPath);
    }
    ...
    {
        // std::cout << "cannot open directory: " << folderPath.c_str() << std::endl;
        return;
    }
    ...
    while ((dirp=readdir(dp))!=NULL)
    {
		...
		// std::cout << "[	stat : ] " << dirp->d_type << " , " << dirp->d_name << ": " << buf.st_mode << '\n';

        // 如果是目录
        if (S_ISDIR(buf.st_mode))
        {
			// std::cout << "[ is dir ] " << dirp->d_name << '\n';
            string path;
			...
			// std::cout << "文件夹路径:" << folderPath << std::endl;
        }
        else
        {
            // 如果是文件直接打印
...
void FileEncrypt::encryptFileByName(string pFileName)
{
    ...
    //CCFileUtils::sharedFileUtils()->getWriteablePath() + pFileName;
