...
int Exercise3() {
    ...
    // 2.1 fragment yellow shader
    int fragmentYellowShader = glCreateShader(GL_FRAGMENT_SHADER);
    glShaderSource(fragmentYellowShader, 1, &fragmentShader2Source, NULL);
...
int main()
{
    ...
    float vertices[] = {
        ...
        -0.5f, 0.75f, 0.0f, // Exec1
        0.5f, 0.75f, 0.0f,  // Exec1
        0.0f, 0.5f, 0.0f,   // Exec1
    };
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3*sizeof(float), (void*)0);
    /*  1. 与着色器中location对应
        2. 顶点属性大小，vec3，由三个值组成，所以是3
        3. 指定数据的类型
        4. 是否希望数据标准化
        5. 步长
        6. 它表示位置数据在缓冲中起始位置的偏移量（Offset）
     */
    glEnableVertexAttribArray(0);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glDrawArrays(GL_TRIANGLES, 0, 6); // Exec1
